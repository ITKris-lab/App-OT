...
interface OrdenDetailScreenProps {
  user: User;
}

export default function OrdenDetailScreen({ user }: OrdenDetailScreenProps) {
  ...
  useEffect(() => {
    const ordenDocRef = doc(db, 'ordenes_trabajo', ticketId);
    ...
    const commentsQuery = query(collection(db, 'ordenes_trabajo', ticketId, 'comments'), orderBy('createdAt', 'asc'));
    ...
    return () => { unsubscribeOrden(); unsubscribeComments(); };
  }, [ticketId]);
  ...
  const handleAddComment = async () => {
    ...
    try {
      await addDoc(collection(db, 'ordenes_trabajo', ticketId, 'comments'), {
        userId: user.id,
        userName: user.name,
  ...
  const handleUpdate = async (dataToUpdate: any) => {
    ...
    const ordenDocRef = doc(db, 'ordenes_trabajo', ticketId);
    try {
      await updateDoc(ordenDocRef, { ...dataToUpdate, updatedAt: serverTimestamp() });
  ...
  const handleDelete = () => {
    ...
    if (Platform.OS === 'web') {
      const confirm = window.confirm("¿Estás seguro de eliminar esta orden? Esta acción no se puede deshacer.");
      if (confirm) {
        performDelete();
      }
    } else {
      Alert.alert("Eliminar Orden", "¿Estás seguro? Esta acción no se puede deshacer.", [
  ...
  const getPriorityColor = (priority: OrdenPriority) => ORDEN_PRIORITIES.find(p => p.value === priority)?.color || '#666';
    ...
    <View>
      <Surface style={styles.header}>
        <View style={styles.headerContent}>
          <View style={styles.titleContainer}>
            <Title style={styles.title}>{orden.title}</Title>
          ...
          {user?.role === 'admin' && (
            <View>
              ...
              >
                ...
                <Divider />
                <Menu.Item leadingIcon="trash-can-outline" onPress={handleDelete} title="Eliminar Orden" titleStyle={{color: '#B00020'}} />
              </Menu>
            </View>
          )}
        </View>
      ...
      <Card style={styles.card}>
        <Card.Content>
          ...
          <Paragraph style={styles.description}>{orden.description}</Paragraph>
